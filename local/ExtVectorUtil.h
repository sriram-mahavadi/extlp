...
private:
    ...
    void _resize_scratch_pad(unsigned int size) {
        m_scratch_pad.resize(size);
    }
    REAL _get_scratch_pad_element(unsigned int index) {
        _validate_scratch_pad(index);
        return m_scratch_pad[index];
    }
    void _set_scratch_pad_element(unsigned int index, REAL value) {
        _validate_scratch_pad(index);
        m_scratch_pad[index] = value;
    }
public:
    //! Dot product of vectors < vct1.vct2 >
    void store_dot_product(PackedVector<REAL>& p_vct1, PackedVector<REAL>& p_vct2, REAL& p_dot_product) {
        bool is_vct2_best_scatter = false;
        // vct1 is not unpacked/scattered already and vct2 is scattered.
        is_vct2_best_scatter = is_vct2_best_scatter || (p_vct1.isPacked() && !p_vct2.isPacked());
        // Both vct1 and vct2 are packed. vct1 has greater sparsity than vct2.
        is_vct2_best_scatter = is_vct2_best_scatter || (p_vct1.getSparsity() > p_vct2.getSparsity());
        // vct2 is a better scatter 
        if (is_vct2_best_scatter) store_dot_product(p_vct2, p_vct1, p_dot_product);
        REAL dot_product_value = 0.0F;
        // vct1 is unpacked
        if (!p_vct1.isPacked()) {
            PackedVector<REAL>::iterator itr = p_vct2.begin();
            while (itr != p_vct2.end()) {
                PackedElement<REAL> &packed_element = *itr;
                REAL vct1_value = p_vct1[packed_element.getIndex()];
                if (vct1_value != 0.0F) {
                    dot_product_value += (vct1_value * packed_element.getValue());
                }
                itr++;
            }
        } else { // vct1 is packed
            ...
            while (itr != p_vct1.end()) {
                PackedElement<REAL> &packed_element = *itr;
                _set_scratch_pad_element(packed_element.getIndex(), packed_element.getValue());
            }
            // Calculating dot product 
            itr = p_vct2.begin();
            while (itr != p_vct2.end()) {
                PackedElement<REAL> &packed_element = *itr;
                REAL vct1_value = _get_scratch_pad_element(packed_element.getIndex());
                if (vct1_value != 0.0F) {
                    dot_product_value += (vct1_value * packed_element.getValue());
                }
            }
            ...
            while (itr != p_vct1.end()) {
                PackedElement<REAL> &packed_element = *itr;
                _set_scratch_pad_element(packed_element.getIndex(), 0.0F);
            }
        }
    ...
    void store_sum(PackedVector<REAL>& p_vct1, PackedVector<REAL>& p_vct2, REAL lambda, PackedVector<REAL>& p_vct_result) {
        bool is_vct2_best_scatter = false;
        // vct1 is not unpacked/scattered already and vct2 is scattered.
        is_vct2_best_scatter = is_vct2_best_scatter || (p_vct1.isPacked() && !p_vct2.isPacked());
        // Both vct1 and vct2 are packed. vct1 has greater sparsity than vct2.
        is_vct2_best_scatter = is_vct2_best_scatter || (p_vct1.getSparsity() > p_vct2.getSparsity());
        // vct2 is a better scatter 
        if (is_vct2_best_scatter) store_sum(p_vct2, p_vct1, 1/lambda, p_vct_result);
        ...
        while (itr != p_vct2.end()) {
            PackedElement<REAL> &packed_element = *itr;
            unsigned int vct2_index = packed_element.getIndex();
            REAL vct1_value = _get_scratch_pad_element();
            REAL vct2_value = packed_element.getValue();
        ...
        while (itr != p_vct1.end()) {
            PackedElement<REAL> &packed_element = *itr;
            unsigned int vct1_index = packed_element.getIndex();
            if(_get_scratch_pad_element(vct1_index)!=0.0F){
                REAL element_value = packed_element.getValue();
